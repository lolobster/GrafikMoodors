...
void main()
{
	...
	if(!normalMapping)
	{
		...
		lightDirection = normalize(lightPosition - fs_in.FragmentPosition); //the light's direction vector is the difference vector between the light's position vector and the fragment's position vector. Öe can easily calculate this difference by subtracting both vectors. LightDirection from the fraagment
		viewDirection = normalize(cameraPosition - fs_in.FragmentPosition); // from a fragment to camera
	}
	else
	{
		if(inTangentSpace)
		{
			normal = texture(material.texture_normal1, fs_in.TexCoords).rgb; // Obtain normal from normal map in range [0,1]
			normal = normalize(normal * 2.0 - 1.0); // Transform normal vector to range [-1,1].  This normal is in tangent space
			lightDirection = normalize(fs_in.TangentLightPos - fs_in.TangentFragPos); //the light's direction vector is the difference vector between the light's position vector and the fragment's position vector. Öe can easily calculate this difference by subtracting both vectors. LightDirection from the fraagment
			viewDirection = normalize(fs_in.TangentCameraPos - fs_in.TangentFragPos); // from a fragment to camera
		}
		else
		{
			//normal = normalize(fs_in.Normal);
	...
	//Specular
	float specularStrength = 0.4f; //a specular intensity value to give the specular highlight a medium-bright color so that it doesn't have too much of an impact
